    ...
    <el-table :data="pagination.records" border>
      ...
      <el-table-column fixed="right" label="操作" width="180">
        <template slot-scope="scope">
          <el-button @click="add(scope.row.paperId,scope.row.courseName)" type="primary" size="small">增加题库</el-button>
          <!-- <el-button @click="clear(scope.row.paperId,scope.row.courseName)" type="danger" size="small">清空</el-button> -->
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
export default {
  ...
  methods: {
    getExamInfo() { //分页查询所有试卷信息
      this.$axios(`/API/getAllExams4Param/${this.pagination.current}/${this.pagination.size}`).then(res => {
    ...
    add(paperId,source) { //增加题库
      this.$router.push({path:'/add2QBChildren',query: {paperId: paperId,subject:source}})
    },
    clear(paperId,source){//清空当前行试卷题库
      console.log(paperId,source)
    }
  },
};
</script>
